...
export function BusinessSection({ articles, onReadMore }: BusinessSectionProps) {
  const businessNews = articles.filter(article => 
    article.category === 'Business' || article.category === 'Finance'
  ).slice(0, 6);

  return (
    <div className="mt-12">
      <div id="business-finance" className="px-6 py-4 border-b-2 border-blue-600 dark:border-blue-500">
        <h2 className="text-xl font-bold uppercase tracking-wide text-gray-800 dark:text-gray-200" style={{color: '#333333'}}>Business & Finance</h2>
      </div>
            ...
            <div key={article.id} className="group cursor-pointer" onClick={() => onReadMore?.(article)}>
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <span className="inline-block px-2 py-1 text-xs font-semibold bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">
                    {article.category}
                  </span>
